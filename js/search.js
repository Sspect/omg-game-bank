...
if (
    ...
    throw new Error(
        "Required display settings elements are missing in games.html",
    );
}
...
function parseIdList(value) {
    ...
    if (Array.isArray(parsedValue)) {
        return parsedValue.map((item) => toIdKey(item)).filter(Boolean);
    }
...
async function loadRelationLookupMaps() {
    ...
    if (mechanicsResult.error) {
        console.error(
            "Error loading mechanics lookup table:",
            mechanicsResult.error,
        );
    }
...
const DISPLAY_FIELDS = [
    ...
    },
    {
        ...
        getValue: (game) => {
            ...
            return formatKnownRange(
                min,
                max,
                "Minimum play time (min)",
                "Maximum play time (min)",
            );
        },
    },
    {
    ...
    },
    {
        ...
        label: "Theme",
        getValue: (game) =>
            formatRelationValues(game.theme, relationLookupMaps.theme),
    },
    {
        key: "mechanics",
        label: "Mechanics",
        getValue: (game) =>
            formatRelationValues(game.mechanics, relationLookupMaps.mechanics),
    },
    {
        key: "language",
        label: "Language",
...
function renderGames() {
    ...
    if (!games.length) {
        gamesList.innerHTML =
            '<div class="col-12"><div class="alert alert-secondary mb-0">No games found.</div></div>';
        return;
    }
    ...
    games.forEach((game) => {
        ...
        const details = DISPLAY_FIELDS.filter(
            (field) =>
